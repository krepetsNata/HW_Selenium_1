...
public class TrainingListPage extends BasePage {
    ...
    public TrainingListPage clickOnSkillsCheckBox(String textItem) {
        waitForPageLoadComplete(20);

        //накладення WebElements
    ...
    public TrainingListPage clickOnCountry(String textItem) {
        ...
        //накладення WebElements
//        JavascriptExecutor executor = (JavascriptExecutor)driver;
//        executor.executeScript("arguments[0].click();", getCheckBoxSkillFromList(textItem));
        waitElementToBeClickable(20, getCountryFromListByName(textItem)).click();
        //waitVisibilityOfElement(20,javaCheckBox);
        //javaCheckBox.click();
    ...
    public TrainingListPage clickOnClearSkillsSpan() {
        ...
        LOG.info("clearSkillsSpan was clicked - filter by skills was cleared");
        return this;
    }
    ...
    public boolean isCheckboxIsSelected(List<WebElement> list, String textItem) {
        ...
        LOG.info(String.format("Is '%s' checkbox selected': '%s'", textItem, isSelected));
        return isSelected;
    }

    public void verifyCheckbosIsChecked(String textItem) { //skills
        Assert.assertTrue(isCheckboxIsSelected(getListSkills(), textItem), String.format("'%s' checkbox is checked", textItem));
    }
    ...
    public void verifyTitleTrainings(String textTitleTraining) {
        if (getListTrainings() != null) {
            boolean flag = false;
            System.out.println("TR LIST SIZE: " + getListTrainings().size());

            for (WebElement el : getListTrainings()) {
                WebElement elTitle = el.findElement(By.xpath("//div[contains(@class,'training-item__title')]"));
                String elText = elTitle.getText().trim();

                if (elText.equalsIgnoreCase(textTitleTraining)) {
                    flag = true;
                } else {
                    flag = false;
                }
                Assert.assertTrue(flag, String.format("'%s' incorrect key word ", textTitleTraining));
            }
        } else {
    ...
    public void verifyCountryTrainings(String textCountryTraining) {
        if (getListTrainings() != null) {
            ...
            for (WebElement el : getListTrainings()) {
                WebElement elLocation = el.findElement(By.xpath("//*[contains(@class,'training-item__location--text')]"));//("//div[@class='training-item__location']"));//
                String elText = elLocation.getText().trim();
                ...
                Assert.assertTrue(flag, String.format("'%s' incorrect key word ", textCountryTraining));
            }
        } else {
            Assert.assertEquals(nullTrainingsSpan.getText(), "No training are available.", "List is empty - trainings are not exist");
        }
